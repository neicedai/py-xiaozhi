...
| `MAX_ACTIVE_PATHS` | `3` | 搜索路径数 | 减少提升速度，增加提升准确性 |
| `KEYWORDS_SCORE` | `2.0` | 关键词增强分数 | 提高减少误检，降低提升灵敏度 |
| `KEYWORDS_THRESHOLD` | `0.25` | 检测阈值 | 降低提升灵敏度，提高减少误检 |
| `NUM_TRAILING_BLANKS` | `1` | 尾随空白数量 | 通常保持1 |
| `ACTIVE_PRESET` | `"balanced"` | 当前启用的预设 | balanced / accuracy / speed / quiet / noisy |

### 预设方案一览

在设置界面中可以直接选择以下预设，系统会自动写入 `ACTIVE_PRESET` 并覆盖相关参数：

| 预设 Key | 界面名称 | 使用场景 | 关键参数 |
| --- | --- | --- | --- |
| `balanced` | 均衡模式（推荐） | 默认推荐，兼顾灵敏度与误触率 | 阈值 0.25 · 分数 2.0 · 路径 3 |
| `accuracy` | 精度优先 | 嘈杂环境，需要更稳的识别 | 阈值 0.28 · 分数 2.4 · 路径 4 |
| `speed` | 速度优先 | 低性能设备，追求低延迟 | 阈值 0.20 · 分数 1.6 · 路径 2 |
| `quiet` | 安静环境 | 会议室/卧室等安静场景 | 阈值 0.22 · 分数 1.8 · 路径 3 |
| `noisy` | 嘈杂环境 | 商场/开放办公区等噪声场景 | 阈值 0.30 · 分数 2.6 · 路径 4 |

> **提示**：如果需要完全自定义参数，只需将 `ACTIVE_PRESET` 留空或在下拉框中手动重新设置数值。
