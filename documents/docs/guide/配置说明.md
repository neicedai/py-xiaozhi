...
| `MAX_ACTIVE_PATHS` | Integer | 3 | 搜索路径数，影响准确性和速度 |
| `KEYWORDS_SCORE` | Float | 2.0 | 关键词增强分数，影响检测灵敏度 |
| `KEYWORDS_THRESHOLD` | Float | 0.25 | 检测阈值，越低越灵敏 |
| `NUM_TRAILING_BLANKS` | Integer | 1 | 尾随空白token数量 |
| `ACTIVE_PRESET` | String | "balanced" | 当前启用的预设模式 |
| `PRESETS` | Object | 见默认配置 | 预设档位集合，可在界面中选择 |
